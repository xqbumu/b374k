...
if (isset($_COOKIE['packer_theme'])) {
	$theme = $_COOKIE['packer_theme'];
} else {
	$theme = "default";
}
...
if (isset($_SERVER['REMOTE_ADDR'])) {
	if (isset($p['read_file'])) {
		...
		foreach ($modules as $module) {
			...
			if (is_file($filename . ".php")) {
				$module_code .= packer_read_file($filename . ".php");
			}
		...
		if ($strip == 'yes') {
			$js_code = packer_pack_js($js_code);
		}
		...
		foreach ($GLOBALS['packer']['theme'] as $k) {
			if ($k == $theme) {
				...
				$available_themes .= "<option>" . $k . "</option>";
			}
	...
	if (count($argv) <= 1) {
		...
		if ($base64 == 'yes') {
			$output .= "Compression level\t: " . $compress_level . "\n";
		}
...
function packer_get_module() {
	...
	foreach (glob($GLOBALS['packer']['module_dir'] . "*.php") as $filename) {
		...
		if (packer_check_module($filename)) {
			$available_modules[] = $filename;
		}
...
function packer_check_module($module) {
	...
	if (is_file($filename . ".php")) {
		...
		@eval("?>" . $content);
		if ($GLOBALS['module'][$module]['id'] == $module) {
			return true;
		}
...
function obscure_php($content, $except) {
	...
	usort($str, $var_sort_fun);

	if (sizeof($str) == sizeof($var)) {
		// 混淆变量
		foreach ($var as $val) {
			// 排除替换变量
...
function packer_b374k($output, $phpcode, $htmlcode, $strip, $base64, $compress, $compress_level, $password, $obscure) {
	...
	if (!empty($password)) {
		$password = "\$GLOBALS['pass'] = \"" . sha1(md5($password)) . "\"; // sha1(md5(pass))\n";
	}
